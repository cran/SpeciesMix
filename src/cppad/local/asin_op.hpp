...
$section Forward Mode Asin Function$$

$head Syntax$$

$syntax%inline void ForAsinOp(size_t %d%,
...
$section Reverse Mode Asin Function$$

$head Syntax$$

$syntax%inline void RevAsinOp(size_t %d%,
...
$th d$$ order Taylor coefficient row vector for $latex \arcsin(x)$$
and $latex 1 + x * x$$ 
...
template <class Base>
inline void ForAsinOp(size_t j, 
	Base *z, Base *b, const Base *x)
{	size_t k;
	Base qj;
...
template <class Base>
inline void RevAsinOp(size_t d, 
	const Base  *z, const Base  *b, const Base *x,
	      Base *pz,       Base *pb,       Base *px)
{	size_t k;
